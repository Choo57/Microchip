...
sbit IRQ0 at RD0_bit;    //INT0 pin
sbit IRQ1 at RC3_bit;    //INT1 pin
//sbit RST at RC5_bit;     //RESET pin
sbit CSCON at RD2_bit;   //CSCON pin
sbit CSDAT at RC1_bit;   //CSDAT pin

sbit IRQ0_Direction at TRISD0_bit;     //IRQ0 pin direction
sbit IRQ1_Direction at TRISC3_bit;     //IRQ1 pin direction
//sbit RST_Direction at TRISC5_bit;      //RST pin direction
sbit CSCON_Direction at TRISD2_bit;    //CSCON pin direction
sbit CSDAT_Direction at TRISC1_bit;    //CSDAT pin direction
...
// SPI1_Write(0x00);                    // Send a dummy byte
...
// Select chip mode
//   Vtune determined by tank inductors values (default)
void set_chip_mode (unsigned short int chip_mode) {
 switch(chip_mode){
  ...
  Delay_ms(50);    // Wait for module stabilization
}

// Wait for the PLL lock
void PLL_lock (void) {
 unsigned short int PLL_locked = 0, reg_status = 0;
// unsigned short int VCOT_select[4] = {0b01010000, 0b01010010, 0b01010100, 0b01010110};  // All in Freq. Synth. mode, increase VCOT value if PLL lock fails
     ...
     do {
         ...
         write_register(0x0E, 0x19);                     // IRQ0TXST (Transmit starts if FIFO is not empty, IRQ0 mapped to FIFOEMPTY), PLL lock enabled, FIFO fill start automatically when sync word detected, Writing: stop filling FIFO
...
//         displayNumber(2,15);                          // Show F and then the PLL_lock value (will show 1 if PLL lock achieved)
//         displayNumber(3, 0);                          // Clear 3rd digit on LCD
//         displayNumber(1, PLL_locked/100);             // Show F and then the PLL_lock value (will show 1 if PLL lock achieved)
         Delay_ms(20);
     } while (PLL_locked == 0);
...
write_register(0x04, 0x0C);    // OOK Floor Threshold = 6dB (default)
 ...
 write_register(0x1C, payload_lngth);    // Manchester Encoding disabled, PLDPLEN payload length (address byte not used)
 ...
 write_register(0x1F, 0b01000000);    // Auto Clear FIFO CRC enabled, read access to FIFO during standby
}